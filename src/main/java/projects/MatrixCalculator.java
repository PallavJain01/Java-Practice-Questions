...
class Exceptions {
  static class InverseNonExistant extends Exception {
    public InverseNonExistant() {
      super("Inverse does not exist (determinant is zero)");
    }
  }
  static class NonComplementary extends Exception {
    public NonComplementary() {
      super("Matrices are not complementary (Different/Invalid order)");
    }
  }
  static class NotSquareMatrix extends Exception {
    public NotSquareMatrix() {
      super("Given matrix is not a square matrix");
    }
  }
}
  ...
  public String matrixToString(int[][] integerMatrix) {
    ...
    for (int i = 0; i < integerMatrix.length; i++) {
      for (int j = 0 ; j < integerMatrix[i].length; j++) {
        matrixString += String.format("%d ", integerMatrix[i][j]);
      }
      matrixString += "]\n[ ";
    }
    matrixString = matrixString.substring(0, matrixString.length() - 3);
    return matrixString;
  }
  ...
  public String matrixToString(float[][] floatMatrix) {
    ...
    for (int i = 0; i < floatMatrix.length; i++) {
      for (int j = 0 ; j < floatMatrix[i].length; j++) {
        matrixString += String.format("%.1f ", floatMatrix[i][j]);
      }
      matrixString += "]\n[ ";
    }
    matrixString = matrixString.substring(0, matrixString.length() - 3);
